"""Load injectable TypeDB connection settings for spiderfeet-actual."""
...
class TypeDBConnectionConfig:
    ...
    def from_dict(cls, raw: Dict[str, Any]) -> "TypeDBConnectionConfig":
        ...
        # Prefer explicit database; default to spiderfeet-actual (not map).
        database = raw.get("database", raw.get("database_name", ACTUAL_DATABASE_NAME))
        tls = raw.get("tls", {}) or {}
    ...
    def with_database(self, database: str) -> "TypeDBConnectionConfig":
        """Return a copy targeting a different database name."""
        return TypeDBConnectionConfig(
