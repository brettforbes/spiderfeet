## Problem
Module `sfp_openstreetmap` has no smoke-validated entry in `module_test_seeds.json` (none-tier corpus gap).

## Last validation
| Field | Value |
|-------|-------|
| consumed | `PHYSICAL_ADDRESS` |
| input | `sbs.com.au` |
| status | `FINISHED` |
| verdict | `clean_miss` |
| produced | `0` |
...
- **Produces:** PHYSICAL_COORDINATES

## Research tasks
1. Inspect `modules/sfp_openstreetmap.py` for required input shape and external API health.
