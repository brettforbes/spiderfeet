## Problem
Module `sfp_gravatar` has no smoke-validated entry in `module_test_seeds.json` (none-tier corpus gap).
...
- **Produces:** RAW_RIR_DATA, USERNAME, EMAILADDR, EMAILADDR_GENERIC, PHONE_NUMBER, GEOINFO, ACCOUNT_EXTERNAL_OWNED, SOCIAL_MEDIA

## Research tasks
1. Inspect `modules/sfp_gravatar.py` for required input shape and external API health.
