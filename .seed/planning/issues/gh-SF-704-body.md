## Problem
Module `sfp_torch` has no smoke-validated entry in `module_test_seeds.json` (none-tier corpus gap).
...
## Research tasks
1. Inspect `modules/sfp_torch.py` for required input shape and external API health.
