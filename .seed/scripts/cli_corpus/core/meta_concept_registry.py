"""SPEC-014 shared meta-concept registry loader (R14-01).

Concept lists, root/category nugget ids, and caps live in
``rules/_shared/narrative_v2.yaml`` — not hard-coded in Python.
"""
...
_REGISTRY_PATH = _SHARED_RULES / "narrative_v2.yaml"
...
def list_meta_concepts(*, path: str | None = None) -> list[dict[str, Any]]:
    """Return meta-concepts sorted by ``order``, each with an ``id`` field."""
...
def get_meta_concept(concept_id: str, *, path: str | None = None) -> dict[str, Any]:
    """Return one concept (with ``id``) or raise KeyError."""
    data = load_narrative_v2(path)
...
def mermaid_settings(*, path: str | None = None) -> dict[str, Any]:
    """Return mermaid caps/mode from the shared registry."""
    data = load_narrative_v2(path)
