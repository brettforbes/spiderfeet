#!/usr/bin/env python3
"""httpx structured JSON helpers and structured→text conversion."""
...
def parse_httpx_structured(raw: str) -> dict[str, Any]:
    ...
    if stripped.startswith("{"):
        ...
        return {"schema": doc.get("schema", HTTPX_STRUCTURED_SCHEMA), "records": records}
    return {"schema": HTTPX_STRUCTURED_SCHEMA, "records": parse_jsonl(stripped)}
...
def build_httpx_bundle(records: list[dict[str, Any]], scan: dict[str, Any] | None = None) -> dict[str, Any]:
    if scan:
        return {
