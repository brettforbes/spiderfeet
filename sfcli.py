...
# -------------------------------------------------------------------------------
# Name:        sfcli
# Purpose:     Command Line Interface for SpiderFeet.
#
...
COPYRIGHT_INFO = "               by Steve Micallef | @spiderFeet\n"
...
class SpiderFeetCli(cmd.Cmd):
    ...
    # Make a request to the SpiderFeet server
    def request(self, url, post=None):
        ...
        headers = {
            "User-agent": "SpiderFeet-CLI/" + self.version,
    ...
    def do_ping(self, line):
        """ping
        Ping the SpiderFeet server to ensure it's responding."""
    ...
    # List all SpiderFeet correlation rules
    ...
    # List all SpiderFeet data element types.
    ...
    def do_load(self, line):
        """load <file>
        Execute SpiderFeet CLI commands found in <file>."""
        pass
    ...
    def print_topics(self, header, cmds, cmdlen, maxcol):
        ...
        helpmap = [
            ...
            ["exit", "Exit the SpiderFeet CLI (won't impact running scans)."],
            ["ping", "Test connectivity to the SpiderFeet server."],
            ...
            ["query", "Run SQL against the SpiderFeet SQLite database."],
    ...
    def do_set(self, line):
        """set [opt [= <val>]]
        Set a configuration variable in SpiderFeet."""
        ...
        if not d:
            self.edprint("Unable to obtain SpiderFeet server-side config.")
            return
        ...
        j = json.loads(d)
        if j[0] == "ERROR":
            self.edprint("Error fetching SpiderFeet server-side config.")
            return
        ...
        if val:
            ...
            if not d:
                self.edprint("Unable to set SpiderFeet server-side config.")
                return

            j = json.loads(d)
            if j[0] == "ERROR":
                self.edprint(f"Error setting SpiderFeet server-side config: {j[1]}")
                return
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description='SpiderFeet: Open Source Intelligence Automation.')
    p.add_argument("-d", "--debug", help="Enable debug output.", action='store_true')
    p.add_argument("-s", metavar="URL", type=str, help="Connect to SpiderFeet server on URL. By default, a connection to http://127.0.0.1:5001 will be attempted.")
    p.add_argument("-u", metavar="USER", type=str, help="Username to authenticate to SpiderFeet server.")
    p.add_argument("-p", metavar="PASS", type=str, help="Password to authenticate to SpiderFeet server. Consider using -P PASSFILE instead so that your password isn't visible in your shell history or in process lists!")
    p.add_argument("-P", metavar="PASSFILE", type=str, help="File containing password to authenticate to SpiderFeet server. Ensure permissions on the file are set appropriately!")
    p.add_argument("-e", metavar="FILE", type=str, help="Execute commands from FILE.")
    p.add_argument("-l", metavar="FILE", type=str, help="Log command history to FILE. By default, history is stored to ~/.spiderFeet_history unless disabled with -n.")
    ...
    if args.l:
        ...
        try:
            s.ownopts['cli.history_file'] = expanduser("~") + "/.spiderFeet_history"
        except BaseException as e:
            s.dprint(f"Failed to set 'cli.history_file': {e}")
            s.dprint("Using '.spiderFeet_history' in working directory")
            s.ownopts['cli.history_file'] = ".spiderFeet_history"
    if args.o:
