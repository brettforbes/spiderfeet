...
def resolve_executable(
    ...
    *,
    ...
    Native/.tools lookup is delegated to
    ``spiderfeet.tools.cli_paths.resolve_cli_binary`` (single source of truth):
    env override → PATH → repo ``.tools/`` layouts. Then optional WSL ``which``.

    Returns ``(prefix_argv, error)``. On success ``error`` is ``None`` and
    ``prefix_argv`` is either ``[path]`` (native) or ``["wsl", name]``.
    """
    found = resolve_cli_binary(name)
    if found:
        return [found], None
