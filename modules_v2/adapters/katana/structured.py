# -*- coding: utf-8 -*-
"""Katana structured JSON helpers and structured→text conversion (modules_v2 port)."""
...
def parse_katana_structured(raw: str) -> dict[str, Any]:
    """Parse katana bundle JSON or legacy JSONL into {schema, records, …}."""
    stripped = raw.strip()
    if not stripped:
    ...
    if stripped.startswith("{"):
        ...
        return {
            "schema": doc.get("schema", KATANA_STRUCTURED_SCHEMA),
            "records": records,
...
def host_from_record(rec: dict[str, Any]) -> str:
