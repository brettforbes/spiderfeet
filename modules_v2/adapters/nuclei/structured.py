# -*- coding: utf-8 -*-
"""Nuclei structured JSON helpers and structured→text conversion (modules_v2 port)."""
...
def parse_jsonl(raw: str) -> list[dict[str, Any]]:
    """Parse Nuclei JSONL / NDJSON stdout into finding dicts."""
...
# Back-compat alias for NDJSON callers.
parse_ndjson = parse_jsonl
...
def parse_nuclei_structured(raw: str) -> dict[str, Any]:
    ...
    if stripped.startswith("{"):
        try:
            ...
            return {"schema": NUCLEI_STRUCTURED_SCHEMA, "records": parse_jsonl(stripped)}
...
def record_to_text_line(rec: dict[str, Any]) -> str:
    ...
    cves = sorted({m.upper() for m in CVE_RE.findall(json.dumps(rec))})
    if cves:
        line += f" [CVE: {', '.join(cves)}]"
    return line
...
def jsonl_to_bundle(jsonl: str, scan: dict[str, Any] | None = None) -> dict[str, Any]:
    return build_nuclei_bundle(parse_jsonl(jsonl), scan)


# Back-compat alias.
ndjson_to_bundle = jsonl_to_bundle
