# -*- coding: utf-8 -*-
"""11B-cited graph hooks for Nuclei structured-native adapter (modules_v2 port)."""
...
from modules_v2.adapters.nuclei.structured import CVE_RE
...
def _extract_cves(record: dict[str, Any]) -> list[str]:
    ...
    if isinstance(cve_id, list):
        for item in cve_id:
            if item:
                cves.add(str(item).upper())
    elif cve_id:
        cves.add(str(cve_id).upper())
    cves.update(m.upper() for m in CVE_RE.findall(json.dumps(record)))
    return sorted(cves)
