...
class sfp_dnsraw(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        for rec in list(recs.keys()):
            ...
            for x in res.answer:
                ...
                evt = SpiderFeetEvent("RAW_DNS_RECORDS", str(x), self.__name__, parentEvent)
                ...
                for rx in list(recs.keys()):
                    ...
                    for m in grps:
                        ...
                        if rx == "MX":
                            evt = SpiderFeetEvent("PROVIDER_MAIL", strdata.lower(), self.__name__, parentEvent)
                            self.notifyListeners(evt)
                            domains.append(strdata.lower())

                        if rx == "NS":
                            evt = SpiderFeetEvent("PROVIDER_DNS", strdata.lower(), self.__name__, parentEvent)
                            self.notifyListeners(evt)
                            domains.append(strdata.lower())

                        if rx == "TXT":
                            evt = SpiderFeetEvent("DNS_TEXT", strdata, self.__name__, parentEvent)
                            self.notifyListeners(evt)

                            if "v=spf" in strdata or "spf2.0/" in strdata:
                                evt = SpiderFeetEvent("DNS_SPF", strdata, self.__name__, parentEvent)
                                self.notifyListeners(evt)
        ...
        for domain in set(domains):
            ...
            evt = SpiderFeetEvent(evt_type, domain, self.__name__, parentEvent)
            self.notifyListeners(evt)
