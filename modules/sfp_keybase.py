...
class sfp_keybase(SpiderFeetPlugin):
    ...
    def handleEvent(self, event) -> None:
        ...
        for user in data:
            ...
            if eventName in ['LINKED_URL_EXTERNAL', 'DOMAIN_NAME']:
                ...
                source_event = SpiderFeetEvent("USERNAME", username, self.__name__, event)
                self.notifyListeners(source_event)
                self.results[username] = True
            else:
                source_event = event

            evt = SpiderFeetEvent("RAW_RIR_DATA", str(user), self.__name__, source_event)
            ...
            if profile:
                ...
                if fullName:
                    evt = SpiderFeetEvent("RAW_RIR_DATA", f"Possible full name: {fullName}", self.__name__, source_event)
                    self.notifyListeners(evt)

                # Get and report location of user
                location = profile.get('location')
                if location:
                    evt = SpiderFeetEvent("GEOINFO", location, self.__name__, source_event)
                    self.notifyListeners(evt)
                ...
                if socialMediaData:
                    for socialMedia in socialMediaData:
                        ...
                        if socialMediaSite and socialMediaURL:
                            ...
                            evt = SpiderFeetEvent("SOCIAL_MEDIA", socialMedia, self.__name__, source_event)
                            self.notifyListeners(evt)
            ...
            if cryptoAddresses:
                ...
                if bitcoinAddresses:
                    for bitcoinAddress in bitcoinAddresses:
                        ...
                        evt = SpiderFeetEvent("BITCOIN_ADDRESS", btcAddress, self.__name__, source_event)
                        self.notifyListeners(evt)
            ...
            for pgpKey in pgpKeys:
                ...
                evt = SpiderFeetEvent("PGP_KEY", pgpKey, self.__name__, source_event)
                self.notifyListeners(evt)
