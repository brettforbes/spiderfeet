...
# -------------------------------------------------------------------------------
# Name:         sfp_spur
# Purpose:      SpiderFeet plugin to search spur.us API for any
...
class sfp_spur(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        for addr in qrylist:
            ...
            if eventName.startswith("NETBLOCK_"):
                ipEvt = SpiderFeetEvent("IP_ADDRESS", addr, self.__name__, event)
                self.notifyListeners(ipEvt)
                evt = SpiderFeetEvent("RAW_RIR_DATA", str(data), self.__name__, ipEvt)
            ...
            if geoTag:
                ...
                if eventName.startswith("NETBLOCK_"):
                    evt = SpiderFeetEvent("GEOINFO", geoInfo, self.__name__, ipEvt)
                    self.notifyListeners(evt)
            ...
            if asData:
                ...
                if orgName:
                    if eventName.startswith("NETBLOCK_"):
                        evt = SpiderFeetEvent("COMPANY_NAME", orgName, self.__name__, ipEvt)
                        self.notifyListeners(evt)
                    elif eventName.startswith("AFFILIATE_"):
                        # Don't report COMPANY_NAME for Affiliates
                        pass
                    else:
                        evt = SpiderFeetEvent("COMPANY_NAME", orgName, self.__name__, event)
                        self.notifyListeners(evt)
            ...
            if vpnOperatorsExists:
                ...
                if eventName.startswith("NETBLOCK_"):
                    evt = SpiderFeetEvent("MALICIOUS_IPADDR", maliciousIPDesc, self.__name__, ipEvt)
                    self.notifyListeners(evt)
                elif eventName.startswith("AFFILIATE_"):
                    evt = SpiderFeetEvent("MALICIOUS_AFFILIATE_IPADDR", maliciousIPDesc, self.__name__, event)
                    self.notifyListeners(evt)
                else:
                    evt = SpiderFeetEvent("MALICIOUS_IPADDR", maliciousIPDesc, self.__name__, event)
                    self.notifyListeners(evt)
