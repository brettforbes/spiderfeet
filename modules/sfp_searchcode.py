...
class sfp_searchcode(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        while page < max_pages:
            ...
            emails = SpiderFeetHelpers.extractEmailsFromText(str(results))
            for email in emails:
            ...
            for result in results:
                ...
                if lines:
                    for line in lines:
                        links.update(SpiderFeetHelpers.extractUrlsFromText(lines[line]))
            ...
            for result in results:
                ...
                evt = SpiderFeetEvent('PUBLIC_CODE_REPO', repo_data, self.__name__, event)
                self.notifyListeners(evt)

                evt = SpiderFeetEvent('RAW_RIR_DATA', json.dumps(result), self.__name__, event)
                self.notifyListeners(evt)
