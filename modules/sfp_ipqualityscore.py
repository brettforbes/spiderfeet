...
class sfp_ipqualityscore(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "PHONE_NUMBER":
            ...
            if data.get('leaked'):
                evt = SpiderFeetEvent("EMAILADDR_COMPROMISED", f"{eventData} [Unknown]", self.__name__, event)
                self.notifyListeners(evt)
            ...
            if malicious:
                ...
                if eventName == "IP_ADDRESS":
                    ...
                    evt = SpiderFeetEvent("MALICIOUS_INTERNET_NAME", maliciousDesc, self.__name__, event)
                self.notifyListeners(evt)
