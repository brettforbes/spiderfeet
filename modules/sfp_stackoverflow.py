...
class sfp_stackoverflow(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        for item in items:
            ...
            emails = SpiderFeetHelpers.extractEmailsFromText(text)
            if emails:
                for email in emails:
        ...
        for email in set(allEmails):
            ...
            if self.getTarget().matches(email):
                e = SpiderFeetEvent('EMAILADDR', email, self.__name__, event)
            else:
                e = SpiderFeetEvent('AFFILIATE_EMAILADDR', email, self.__name__, event)
            self.notifyListeners(e)

        for username in set(allUsernames):
            if " " in username:
                e = SpiderFeetEvent('RAW_RIR_DATA', 'Possible full name: ' + username, self.__name__, event)
            else:
                e = SpiderFeetEvent('USERNAME', username, self.__name__, event)
            self.notifyListeners(e)

        for ip in set(allIP4s):
            ip = str(ip)
            e = SpiderFeetEvent('AFFILIATE_IP_ADDRESS', ip, self.__name__, event)
            self.notifyListeners(e)

        for ip in set(allIP6s):
            ip = str(ip)
            e = SpiderFeetEvent('AFFILIATE_IPV6_ADDRESS', ip, self.__name__, event)
            self.notifyListeners(e)
