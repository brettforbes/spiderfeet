...
class sfp_xforce(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        for addr in qrylist:
            ...
            if rec:
                ...
                if len(rec_history) > 0:
                    ...
                    e = SpiderFeetEvent("RAW_RIR_DATA", str(rec_history), self.__name__, event)
                    ...
                    for result in rec_history:
                        ...
                        e = SpiderFeetEvent(malicious_type, text, self.__name__, event)
                        self.notifyListeners(e)
                        e = SpiderFeetEvent(blacklist_type, text, self.__name__, event)
                        self.notifyListeners(e)
            ...
            if rec:
                ...
                if len(rec_malware) > 0:
                    ...
                    e = SpiderFeetEvent("RAW_RIR_DATA", str(rec_malware), self.__name__, event)
                    self.notifyListeners(e)
        ...
        if eventName in ["IP_ADDRESS", "IPV6_ADDRESS"]:
            ...
            e = SpiderFeetEvent("RAW_RIR_DATA", str(records), self.__name__, event)
            self.notifyListeners(e)
                    ...
                    if not self.opts["cohostsamedomain"]:
                        if self.getTarget().matches(host, includeParents=True):
                            if self.sf.resolveHost(host) or self.sf.resolveHost6(host):
                                e = SpiderFeetEvent("INTERNET_NAME", host, self.__name__, event)
                            else:
                                e = SpiderFeetEvent("INTERNET_NAME_UNRESOLVED", host, self.__name__, event)
                            self.notifyListeners(e)

                            if self.sf.isDomain(host, self.opts['_internettlds']):
                                e = SpiderFeetEvent("DOMAIN_NAME", host, self.__name__, event)
                                self.notifyListeners(e)
                            continue

                    e = SpiderFeetEvent("CO_HOSTED_SITE", host, self.__name__, event)
                    self.notifyListeners(e)
                    self.cohostcount += 1
