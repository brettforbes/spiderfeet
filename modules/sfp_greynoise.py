...
class sfp_greynoise(SpiderFeetPlugin):
    ...
    def queryIP(self, qry, qry_type):
        ...
        if qry_type == "ip":
            ...
            ip_response = self.sf.fetchUrl(
                ...
                useragent="greynoise-spiderFeet-v1.2.0",
                headers=headers,
            )
            ...
            query_response = self.sf.fetchUrl(
                gn_gnql_url + qry,
                timeout=self.opts["_fetchtimeout"],
                useragent="greynoise-spiderFeet-v1.1.0",
                headers=headers,
            )
                    ...
                    if rec.get("metadata") and eventName == "IP_ADDRESS":
                        ...
                        if met.get("country", "unknown") != "unknown":
                            ...
                            if met.get("city"):
                                loc = met.get("city") + ", "
                            loc += met.get("country")
                            e = SpiderFeetEvent("GEOINFO", loc, self.__name__, event)
                            self.notifyListeners(e)
                        if met.get("asn", "unknown") != "unknown":
                            asn = met.get("asn").replace("AS", "")
                            e = SpiderFeetEvent("BGP_AS_MEMBER", asn, self.__name__, event)
                            self.notifyListeners(e)
                        if met.get("organization", "unknown") != "unknown":
                            e = SpiderFeetEvent("COMPANY_NAME", met.get("organization"), self.__name__, event)
                            self.notifyListeners(e)
                        if met.get("os", "unknown") != "unknown":
                            e = SpiderFeetEvent("OPERATING_SYSTEM", met.get("os"), self.__name__, event)
                            self.notifyListeners(e)
                        e = SpiderFeetEvent("RAW_RIR_DATA", str(rec), self.__name__, event)
                        self.notifyListeners(e)
