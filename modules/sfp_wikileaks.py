...
class sfp_wikileaks(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        links = dict()
        p = SpiderFeetHelpers.extractLinksFromHtml(wlurl, res['content'], "wikileaks.org")
        if p:
            links.update(p)

        p = SpiderFeetHelpers.extractLinksFromHtml(wlurl, res['content'], "cryptome.org")
        if p:
            links.update(p)
            ...
            for link in links:
                ...
                if link.count('/') >= 4:
                    if not link.endswith(".js") and not link.endswith(".css"):
                        evt = SpiderFeetEvent("LEAKSITE_URL", link, self.__name__, event)
