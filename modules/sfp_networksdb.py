...
class sfp_networksdb(SpiderFeetPlugin):
            ...
            if data is None:
                ...
                if network:
                    ...
                    if cidr and cidr != 'N/A' and self.sf.validIpNetwork(cidr):
                        if ":" in cidr:
                            evt = SpiderFeetEvent('NETBLOCKV6_MEMBER', cidr, self.__name__, event)
                        else:
                            evt = SpiderFeetEvent('NETBLOCK_MEMBER', cidr, self.__name__, event)
                        self.notifyListeners(evt)
        ...
        if eventName in ["INTERNET_NAME", "DOMAIN_NAME"]:
            ...
            for ip in res:
                if self.sf.validIP(ip):
                    evt = SpiderFeetEvent('IP_ADDRESS', ip, self.__name__, event)
                    self.notifyListeners(evt)
                elif self.sf.validIP6(ip):
                    evt = SpiderFeetEvent('IPV6_ADDRESS', ip, self.__name__, event)
                    self.notifyListeners(evt)
