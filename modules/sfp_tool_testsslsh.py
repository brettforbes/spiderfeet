...
# Purpose:     SpiderFeet plug-in for using the testssl.sh tool.
...
class sfp_tool_testsslsh(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        if not SpiderFeetHelpers.sanitiseInput(eventData, extra=['/']):
        ...
        for target in targets:
            ...
            if eventName == 'NETBLOCK_OWNER':
                ...
                if generate:
                    pevent = SpiderFeetEvent("IP_ADDRESS", target, self.__name__, event)
            ...
            for result in result_json:
                ...
                if 'cve' in result:
                    ...
                    evt = SpiderFeetEvent("VULNERABILITY_GENERAL", f"{result['id']} ({result['finding']})", self.__name__, pevent)
                    self.notifyListeners(evt)
