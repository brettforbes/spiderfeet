...
class sfp_crxcavator(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        evt = SpiderFeetEvent('RAW_RIR_DATA', json.dumps(results), self.__name__, event)
        ...
        for result in results:
            ...
            evt = SpiderFeetEvent('RAW_RIR_DATA', json.dumps(extensions), self.__name__, event)
            self.notifyListeners(evt)
        ...
        for url in set(urls):
            ...
            if self.getTarget().matches(host, includeChildren=True, includeParents=True):
                evt = SpiderFeetEvent('LINKED_URL_INTERNAL', url, self.__name__, event)
                self.notifyListeners(evt)
