...
class sfp_template(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        # The three most used fields in SpiderFeetEvent are:
        ...
        for addr in qrylist:
            ...
            os = rec.get('os')
            if os:
                evt = SpiderFeetEvent("OPERATING_SYSTEM", f"{os} ({addr})", self.__name__, pevent)
                self.notifyListeners(evt)
