...
class sfp_abstractapi(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "DOMAIN_NAME":
            ...
            e = SpiderFeetEvent("COMPANY_NAME", name, self.__name__, event)
            self.notifyListeners(e)
            ...
            if linkedin_url:
                ...
                e = SpiderFeetEvent("SOCIAL_MEDIA", f"LinkedIn (Company): <SFURL>{linkedin_url}</SFURL>", self.__name__, event)
                self.notifyListeners(e)
            ...
            if geoinfo:
                e = SpiderFeetEvent("GEOINFO", geoinfo, self.__name__, event)
                self.notifyListeners(e)
            ...
            if carrier:
                e = SpiderFeetEvent("PROVIDER_TELCO", carrier, self.__name__, event)
                self.notifyListeners(e)
