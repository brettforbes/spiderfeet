...
class sfp_hackertarget(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == 'DOMAIN_NAME_PARENT':
            ...
            evt = SpiderFeetEvent('RAW_DNS_RECORDS', "\n".join(records), self.__name__, event)
                ...
                for host in set(hosts):
                    ...
                    if self.sf.isDomain(host, self.opts['_internettlds']):
                        if evt_type.startswith('AFFILIATE'):
                            evt = SpiderFeetEvent('AFFILIATE_DOMAIN_NAME', host, self.__name__, event)
                            self.notifyListeners(evt)
                        else:
                            evt = SpiderFeetEvent('DOMAIN_NAME', host, self.__name__, event)
                            self.notifyListeners(evt)
        ...
        for ip in qrylist:
                ...
                if h not in myres and h != ip:
                    ...
                    if self.cohostcount < self.opts['maxcohost']:
                        ...
                        if eventName == "NETBLOCK_OWNER":
                            ...
                            evt = SpiderFeetEvent("CO_HOSTED_SITE", h.lower(), self.__name__, ipe)
                            self.notifyListeners(evt)
                        else:
                            evt = SpiderFeetEvent("CO_HOSTED_SITE", h.lower(), self.__name__, event)
                            self.notifyListeners(evt)
            ...
            if eventName == 'NETBLOCK_OWNER':
                pevent = SpiderFeetEvent("IP_ADDRESS", ip, self.__name__, event)
            ...
            if self.opts.get('http_headers', True):
                ...
                if http_headers is not None:
                    e = SpiderFeetEvent('WEBSERVER_HTTPHEADERS', json.dumps(http_headers), self.__name__, pevent)
                    e.actualSource = ip
