...
class sfp_robtex(SpiderFeetPlugin):
        ...
        for ip in qrylist:
            ...
            for r in data.get('pas'):
                ...
                if eventName == "NETBLOCK_OWNER":
                    ...
                    evt = SpiderFeetEvent("CO_HOSTED_SITE", host, self.__name__, ipe)
                    self.notifyListeners(evt)
                elif eventName == "NETBLOCKV6_OWNER":
                    ipe = SpiderFeetEvent("IPV6_ADDRESS", ip, self.__name__, event)
                    self.notifyListeners(ipe)
                    evt = SpiderFeetEvent("CO_HOSTED_SITE", host, self.__name__, ipe)
                    self.notifyListeners(evt)
                else:
                    evt = SpiderFeetEvent("CO_HOSTED_SITE", host, self.__name__, event)
                    self.notifyListeners(evt)
