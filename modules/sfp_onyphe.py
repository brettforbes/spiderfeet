...
# -------------------------------------------------------------------------------
# Name:         sfp_onyphe
# Purpose:      SpiderFeet plug-in to check if the IP is included on Onyphe
...
class sfp_onyphe(SpiderFeetPlugin):
    ...
    def emitLocationEvent(self, location, eventData, event):
        ...
        evt = SpiderFeetEvent("PHYSICAL_COORDINATES", location, self.__name__, event)
    ...
    def emitDomainData(self, response, eventData, event):
        ...
        for domain in domains:
            ...
            if self.cohostcount < self.opts['maxcohost']:
                ...
                evt = SpiderFeetEvent("CO_HOSTED_SITE", domain, self.__name__, event)
                self.notifyListeners(evt)
                self.cohostcount += 1
