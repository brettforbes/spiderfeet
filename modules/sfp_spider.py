...
# -------------------------------------------------------------------------------
# Name:         sfp_spider
# Purpose:      SpiderFeet plug-in for spidering sites and returning meta data
...
class sfp_spider(SpiderFeetPlugin):
    ...
    def processUrl(self, url: str) -> dict:
        ...
        links = SpiderFeetHelpers.extractLinksFromHtml(
            url,
            data,
        ...
        for link in links:
            if not self.opts['reportduplicates']:
                if link in self.urlEvents:
                    continue
            # Supply the SpiderFeetEvent of the parent URL as the parent
    ...
    def cleanLinks(self, links: list) -> list:
        ...
        for link in links:
            linkBase = SpiderFeetHelpers.urlBaseUrl(link)
    ...
    def linkNotify(self, url: str, parentEvent=None):
        ...
        event = SpiderFeetEvent(utype, url, self.__name__, parentEvent)
    ...
    def spiderFrom(self, startingPoint: str) -> None:
        ...
        if self.opts['robotsonly']:
            targetBase = SpiderFeetHelpers.urlBaseUrl(startingPoint)
            if targetBase not in self.robotsRules:
                ...
                if res:
                    ...
                    if robots_txt:
                        ...
                        self.robotsRules[targetBase] = SpiderFeetHelpers.extractUrlsFromRobotsTxt(robots_txt)
