...
# -------------------------------------------------------------------------------
# Name:         sfp_botscout
# Purpose:      SpiderFeet plug-in to search botsout.com using their API, for
...
class sfp_botscout(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "IP_ADDRESS":
            ...
            evt = SpiderFeetEvent("MALICIOUS_IPADDR", text, self.__name__, event)
            self.notifyListeners(evt)

            evt = SpiderFeetEvent("BLACKLISTED_IPADDR", text, self.__name__, event)
            self.notifyListeners(evt)
            ...
            evt = SpiderFeetEvent("MALICIOUS_EMAILADDR", text, self.__name__, event)
            self.notifyListeners(evt)
        else:
