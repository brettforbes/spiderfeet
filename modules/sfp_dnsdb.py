...
class sfp_dnsdb(SpiderFeetPlugin):
    meta = {
        "name": "DNSDB",
    ...
    def handleEvent(self, event):
        ...
        if eventName == "DOMAIN_NAME":
            ...
            evt = SpiderFeetEvent("RAW_RIR_DATA", str(rrsetRecords), self.__name__, event)
            ...
            for record in rrsetRecords:
                ...
                for data in record.get("rdata"):
                    ...
                    if record.get("rrtype") == "A":
                        ...
                        evt = SpiderFeetEvent("IP_ADDRESS", data, self.__name__, event)
                    ...
                    if record.get("rrtype") == "AAAA":
                        ...
                        evt = SpiderFeetEvent("IPV6_ADDRESS", data, self.__name__, event)
                    elif record.get("rrtype") == "MX":
                        data = re.sub(r'.*\s+(.*)', r'\1', data)
                        evt = SpiderFeetEvent("PROVIDER_MAIL", data, self.__name__, event)
                    elif record.get("rrtype") == "NS":
                        evt = SpiderFeetEvent("PROVIDER_DNS", data, self.__name__, event)
                    elif record.get("rrtype") == "TXT":
                        data = data.replace('"', '')
                        evt = SpiderFeetEvent("DNS_TEXT", data, self.__name__, event)
            ...
            evt = SpiderFeetEvent("RAW_RIR_DATA", str(rdataRecords), self.__name__, event)
            self.notifyListeners(evt)
            ...
            for record in rdataRecords:
                ...
                if self.opts["verify"] and not self.sf.resolveHost(data) and not self.sf.resolveHost6(data):
                    ...
                    evt = SpiderFeetEvent("INTERNET_NAME_UNRESOLVED", data, self.__name__, event)
                else:
                    evt = SpiderFeetEvent("INTERNET_NAME", data, self.__name__, event)
                self.notifyListeners(evt)
