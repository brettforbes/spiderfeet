...
class sfp_mnemonic(SpiderFeetPlugin):
                ...
                if eventName in ['INTERNET_NAME', 'DOMAIN_NAME']:
                    ...
                    if r['rrtype'] == 'a':
                        ...
                        if self.sf.isValidLocalOrLoopbackIp(answer):
                            evt = SpiderFeetEvent("INTERNAL_IP_ADDRESS", answer, self.__name__, event)
                        else:
                            evt = SpiderFeetEvent("IP_ADDRESS", answer, self.__name__, event)
                    ...
                    if r['rrtype'] == 'aaaa':
                        ...
                        if self.sf.isValidLocalOrLoopbackIp(answer):
                            ...
                            evt = SpiderFeetEvent("IPV6_ADDRESS", answer, self.__name__, event)
                        self.notifyListeners(evt)
        ...
        for co in set(cohosts):
            ...
            if self.getTarget().matches(co, includeParents=True):
                if self.opts['verify'] and not self.sf.resolveHost(co) and not self.sf.resolveHost6(co):
                    ...
                    evt = SpiderFeetEvent("INTERNET_NAME_UNRESOLVED", co, self.__name__, event)
                    self.notifyListeners(evt)
                    continue
