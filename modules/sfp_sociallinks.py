...
class sfp_sociallinks(SpiderFeetPlugin):
        ...
        if eventName == "PHONE_NUMBER":
            ...
            if resultSet:
                if resultSet.get('first_name') and resultSet.get('last_name'):
                    evt = SpiderFeetEvent("HUMAN_NAME", f"{resultSet.get('first_name')} {resultSet.get('last_name')}", self.__name__, event)
                    self.notifyListeners(evt)
                if resultSet.get('username'):
                    evt = SpiderFeetEvent("USERNAME", resultSet.get('username'), self.__name__, event)
                    self.notifyListeners(evt)
            ...
            if resultSet:
                ...
                if resultSet.get('phone_number'):
                    evt = SpiderFeetEvent("PHONE_NUMBER", resultSet.get('phone_number'), self.__name__, event)
                    self.notifyListeners(evt)
            ...
            if data is None:
                ...
                if resultSet:
                    ...
                    if resultSet.get('profileurl').get('_content'):
                        evt = SpiderFeetEvent("SOCIAL_MEDIA", f"Flickr: <SFURL>{resultSet.get('profileurl').get('_content')}</SFURL>", self.__name__, event)
                        self.notifyListeners(evt)
            ...
            if data is None:
                ...
                if resultSet:
                    ...
                    if resultSet.get('skypeId'):
                        evt = SpiderFeetEvent("ACCOUNT_EXTERNAL_OWNED", f"Skype [{resultSet.get('skypeId')}]", self.__name__, event)
                        self.notifyListeners(evt)
                        evt = SpiderFeetEvent("USERNAME", resultSet.get('skypeId'), self.__name__, event)
                        self.notifyListeners(evt)
            ...
            if data is None:
                ...
                if resultSet:
                    ...
                    if resultSet.get('companyName'):
                        evt = SpiderFeetEvent("COMPANY_NAME", resultSet.get('companyName'), self.__name__, event)
                        self.notifyListeners(evt)
                    if resultSet.get('headline'):
                        evt = SpiderFeetEvent("JOB_TITLE", resultSet.get('headline'), self.__name__, event)
                        self.notifyListeners(evt)

                    evt = SpiderFeetEvent("SOCIAL_MEDIA", f"LinkedIn: <SFURL>{resultSet.get('linkedInUrl')}</SFURL>", self.__name__, event)
                    self.notifyListeners(evt)
            ...
            for humanName in humanNames:
                evt = SpiderFeetEvent("HUMAN_NAME", humanName, self.__name__, event)
                self.notifyListeners(evt)
