...
class sfp_numverify(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        if data.get('country_code'):
            country = SpiderFeetHelpers.countryNameFromCountryCode(data.get('country_code'))
        ...
        if data.get('carrier'):
            evt = SpiderFeetEvent("PROVIDER_TELCO", data.get('carrier'), self.__name__, event)
            self.notifyListeners(evt)
        else:
