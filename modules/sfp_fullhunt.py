...
class sfp_fullhunt(SpiderFeetPlugin):
    ...
    def handleEvent(self, event):
        ...
        for record in res:
            ...
            if network_ports:
                for port in network_ports:
                    e = SpiderFeetEvent("TCP_PORT_OPEN", f"{host}:{port}", self.__name__, event)
                    self.notifyListeners(e)
        ...
        for host in set(mail_servers):
            ...
            e = SpiderFeetEvent("PROVIDER_MAIL", host, self.__name__, event)
            self.notifyListeners(e)
        ...
        for host in set(name_servers):
            ...
            e = SpiderFeetEvent("PROVIDER_DNS", host, self.__name__, event)
            self.notifyListeners(e)
