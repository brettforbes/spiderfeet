...
Vanilla HTML5 + JavaScript only. Each chart is a function `renderX(svgElement, data, options)` — no React hooks.

```javascript
function renderTree(svgElement, data) {
  if (!data) return;
...
function LinkedCharts({ data }) {
  ...
  function renderChart(svgElement, data) {
    ...
    const svg1 = d3.select(svg1Element);
    ...
    // Chart 2: Bar chart
    const svg2 = d3.select(svg2Element);
