...
Every examination has one SCAN_RECORD with scan descriptors linked via had. This scan includes **1** Scan root node(s) (e.g. `nuclei:http://scanme.nmap.org:nuclei -u http://scanme.nmap.org -silent -jsonl -omit-raw -omit-template -t .tools/nuclei-templates -no-interactsh -etags dos,fuzz,misc -duc -retries 1 -c 25 -timeout 10 -jle .docs/docs-for-cli-tools/exploration_scratch/nuclei/scanme_all_templates.jsonl`). Linked structures: `SCAN_CLI`, `SCAN_TARGET`, `SCAN_START`, `SCAN_ELAPSED`, `SCAN_EXIT_STATUS`, `SCAN_FINDING_COUNT`.
...
flowchart TD
  scan_record_1["SCAN_RECORD"]
  scan_cli_2["SCAN_CLI: nuclei -u http://scanme.nmap.org -silen…"]
...
flowchart TD
  scan_record_1["SCAN_RECORD"]
  scan_target_2["SCAN_TARGET: http://scanme.nmap.org"]
...
flowchart TD
  scan_record_1["SCAN_RECORD"]
  scan_start_2["SCAN_START: 2026-07-05T11:58:42.182007+00:00"]
...
flowchart TD
  scan_record_1["SCAN_RECORD"]
  scan_finding_count_2["SCAN_FINDING_COUNT: 25"]
...
Qualified HOST endpoints own category trees for networks, applications, environment, and security findings. This scan includes **2** Host root node(s) (e.g. `http://scanme.nmap.org`, `scanme.nmap.org`). Linked structures: `SECURITY`.
...
flowchart TD
  security_1["SECURITY"]
  findings_2["FINDINGS: scanme.nmap.org::FINDINGS"]
  security_1 -->|contains| findings_2
  templates_used_3["TEMPLATES_USED: scanme.nmap.org::TEMPLATES_USED"]
...
APPLICATION services listen-to PORT entities under NETWORKS/TRANSPORT. This scan includes **2** Services and ports root node(s) (e.g. `scanme.nmap.org:80`, `scanme.nmap.org:22`). Linked structures: no child categories.
...
SECURITY under HOST holds FINDINGS severity buckets, template inventory, and vulnerability observations. This scan includes **1** Security findings root node(s) (e.g. `scanme.nmap.org::SECURITY`). Linked structures: `TEMPLATES_USED`, `FINDINGS`.
...
flowchart TD
  ...
  nuclei_template_2["NUCLEI_TEMPLATE: CVE-2023-48795"]
  templates_used_1 -->|contains| nuclei_template_2
  nuclei_template_3["NUCLEI_TEMPLATE: aaaa-fingerprint"]
  templates_used_1 -->|contains| nuclei_template_3
  nuclei_template_4["NUCLEI_TEMPLATE: apache-detect"]
...
flowchart TD
  findings_1["FINDINGS"]
  nuclei_severity_info_2["NUCLEI_SEVERITY_INFO: scanme.nmap.org::NUCLEI_SEVERITY_INFO"]
  findings_1 -->|contains| nuclei_severity_info_2
  nuclei_severity_low_3["NUCLEI_SEVERITY_LOW: scanme.nmap.org::NUCLEI_SEVERITY_LOW"]
  findings_1 -->|contains| nuclei_severity_low_3
  nuclei_severity_medium_4["NUCLEI_SEVERITY_MEDIUM: scanme.nmap.org::NUCLEI_SEVERITY_MEDIUM"]
  findings_1 -->|contains| nuclei_severity_medium_4
```
