...
Every examination has one SCAN_RECORD with scan descriptors linked via had. This scan includes **1** Scan root node(s) (e.g. `nuclei:pentest-ground.com:6379:nuclei -u pentest-ground.com:6379 -silent -jsonl -omit-raw -omit-template -t .tools/nuclei-templates/network/cves/2022/CVE-2022-0543.yaml -t .tools/nuclei-templates/network/exposures/exposed-redis.yaml -no-interactsh -etags dos,fuzz,misc -duc -retries 1 -c 10 -timeout 15 -jle .docs/docs-for-cli-tools/exploration_scratch/nuclei/cipherheart_redis_lab.jsonl`). Linked structures: `SCAN_CLI`, `SCAN_TARGET`, `SCAN_START`, `SCAN_ELAPSED`, `SCAN_EXIT_STATUS`, `SCAN_FINDING_COUNT`.
...
flowchart TD
  scan_record_1["SCAN_RECORD"]
  scan_cli_2["SCAN_CLI: nuclei -u pentest-ground.com:6379 -sile…"]
...
flowchart TD
  scan_record_1["SCAN_RECORD"]
  scan_target_2["SCAN_TARGET: pentest-ground.com:6379"]
...
flowchart TD
  scan_record_1["SCAN_RECORD"]
  scan_start_2["SCAN_START: 2026-07-05T11:58:42.228637+00:00"]
...
flowchart TD
  scan_record_1["SCAN_RECORD"]
  scan_finding_count_2["SCAN_FINDING_COUNT: 2"]
...
Qualified HOST endpoints own category trees for networks, applications, environment, and security findings. This scan includes **2** Host root node(s) (e.g. `pentest-ground.com:6379`, `pentest-ground.com`). Linked structures: `SECURITY`.
...
APPLICATION services listen-to PORT entities under NETWORKS/TRANSPORT. This scan includes **1** Services and ports root node(s) (e.g. `pentest-ground.com:6379`). Linked structures: no child categories.
...
flowchart TD
  ...
  nuclei_template_2["NUCLEI_TEMPLATE: CVE-2022-0543"]
  templates_used_1 -->|contains| nuclei_template_2
  nuclei_template_3["NUCLEI_TEMPLATE: exposed-redis"]
...
| `NUCLEI_VULN_DESCRIPTION` | `This template exploits CVE-2022-0543, a Lua-based Redis sandbox escape. The vulnerability was introduced by Debian and Ubuntu Redis packages that insufficiently sanitized the Lua environment. The maintainers failed to disable the package interface, allowing attackers to load arbitrary libraries. ` |
