...
The scan was executed with **nmap** version **7.80**, targeting **scanme.nmap.org** from **Tue Jun 23 19:03:03 2026**. The operator invoked: `nmap -sT -A -T3 --top-ports 1000 --open -oX - scanme.nmap.org`.
 The run completed in **102.42** seconds.

Nmap done at Tue Jun 23 19:04:45 2026; 1 IP address (1 host up) scanned in 102.42 seconds
...
- **Os Type** (`general purpose`)
- **Os Vendor** (`Linux`)
The host environment indicates operating system **Linux 2.6.35**.
- **Accuracy** (`89`)
- **Os Family** (`Linux`)
- **Os Gen** (`2.6.X`)
- **Os Type** (`general purpose`)
...
- **Os Type** (`general purpose`)
- **Os Vendor** (`Linux`)
The host environment indicates operating system **Linux 4.8**.
- **Accuracy** (`90`)
- **Os Family** (`Linux`)
- **Os Gen** (`4.X`)
- **Os Type** (`general purpose`)
...
- Port **32689** on **udp** is **closed**, associated with **an unnamed service**.
...
1. Hop **1** (TTL **2**, RTT **7.00 ms**) reaches **203.134.80.60**.
2. Hop **2** (TTL **4**, RTT **8.00 ms**) reaches **203.134.80.236**.
3. Hop **3** (TTL **5**, RTT **8.00 ms**) reaches **175.45.103.109**.
4. Hop **4** (TTL **6**, RTT **158.00 ms**) reaches **114.31.192.64**.
5. Hop **5** (TTL **7**, RTT **158.00 ms**) reaches **114.31.199.41**.
6. Hop **6** (TTL **8**, RTT **158.00 ms**) reaches **114.31.199.249**.
7. Hop **7** (TTL **9**, RTT **160.00 ms**) reaches **206.223.116.196**.
8. Hop **8** (TTL **13**, RTT **162.00 ms**) reaches **45.33.32.156**.
...
| DESCRIPTOR | HOP_RTT | Trace Hop RTT | `158.00` |
| DESCRIPTOR | HOP_RTT | Trace Hop RTT | `160.00` |
| DESCRIPTOR | HOP_RTT | Trace Hop RTT | `162.00` |
| DESCRIPTOR | HOP_RTT | Trace Hop RTT | `7.00` |
...
| DESCRIPTOR | OPERATING_SYSTEM | Operating System | `Linux 2.6.35` |
...
| DESCRIPTOR | OPERATING_SYSTEM | Operating System | `Linux 4.8` |
...
| DESCRIPTOR | SCAN_ELAPSED | Scan Elapsed Time | `102.42` |
| DESCRIPTOR | SCAN_START | Scan Start | `Tue Jun 23 19:03:03 2026` |
| DESCRIPTOR | SCAN_SUMMARY | Scan Summary | `Nmap done at Tue Jun 23 19:04:45 2026; 1 IP address (1 host up) scanned in 102.42 seconds` |
...
| ENTITY | SCAN_RECORD | Scan Record | `nmap:scanme.nmap.org:Tue Jun 23 19:03:03 2026` |
...
| SUBENTITY | CPE_URL | CPE URL | `cpe:/o:linux:linux_kernel:2.6.35` |
...
| SUBENTITY | CPE_URL | CPE URL | `cpe:/o:linux:linux_kernel:4.8` |
...
| SUBENTITY | PORT | Network Port | `32689` |
...
*OS-Intel Scan · Tue Jun 23 19:03:03 2026 · Page 1*
