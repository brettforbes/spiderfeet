...
from spiderfeet import SpiderFeetDb, SpiderFeetEvent, SpiderFeetPlugin, SpiderFeetTarget, SpiderFeetHelpers, SpiderFeetThreadPool, SpiderFeetCorrelator, logger


def startSpiderFeetScanner(loggingQueue, *args, **kwargs):
...
class SpiderFeetScanner():
    ...
    def __init__(self, scanName: str, scanId: str, targetValue: str, targetType: str, moduleList: list, globalOpts: dict, start: bool = True, seedPayloadEvent: tuple | None = None) -> None:
        ...
        self.__dbh = SpiderFeetDb(self.__config)
        ...
        if scanId:
            ...
            self.__scanId = SpiderFeetHelpers.genScanInstanceId()
        ...
        # Create our target
        try:
            self.__target = SpiderFeetTarget(self.__targetValue, self.__targetType)
        ...
        self.__sharedThreadPool = SpiderFeetThreadPool(threads=self.__config.get("_maxthreads", 3), name='sharedThreadPool')
    ...
    def __startScan(self) -> None:
        ...
        try:
            ...
            rootEvent = SpiderFeetEvent("ROOT", self.__targetValue, "", None)
            psMod.notifyListeners(rootEvent)
            firstEvent = SpiderFeetEvent(self.__targetType, self.__targetValue,
    ...
    def runCorrelations(self) -> None:
        ...
        corr = SpiderFeetCorrelator(self.__dbh, ruleset, self.__scanId)
